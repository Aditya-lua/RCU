...
local Safe        = loadstring(game:HttpGet(SHARED_ROOT .. "SafeHumanoid.lua"))()

-- Re-export services for compatibility with the rest of this file
...
local round = TableUtils.round
local isnil = TableUtils.isnil
Number = math.random(1, 1000000)

local function notify(t, d, s)
    UIHelpers.notify(Library, t, d, s)
end

notify("Versus Airlines", "Loading... Do not enable features yet.", "info")

task.spawn(function()
    pcall(function()
        if game:GetService("ReplicatedStorage"):FindFirstChild("Effect") then
            local effectFolder = game:GetService("ReplicatedStorage").Effect
            if effectFolder:FindFirstChild("Container") then
                for _, v in pairs(effectFolder.Container:GetChildren()) do
    ...
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("Effect", 10).ChildAdded:Connect(function(child)
            if child.Name == "Container" then
                child.ChildAdded:Connect(function(effect)
    ...
    pcall(function()
        if game:GetService("ReplicatedStorage"):FindFirstChild("Effect") and game:GetService("ReplicatedStorage").Effect:FindFirstChild("Container") then
            game:GetService("ReplicatedStorage").Effect.Container.ChildAdded:Connect(function(child)
                if child.Name == "Death" then
                    task.wait(0.1)
...
-- Safe humanoid helpers — delegated to shared/SafeHumanoid.lua
local SafeGetHumanoid      = Safe.getHumanoid
local SafeHumanoidHealth   = Safe.health
local SafeHumanoidMaxHealth= Safe.maxHealth
local SafeSetWalkSpeed     = Safe.setWalkSpeed
local SafeSetJumpPower     = Safe.setJumpPower
local SafeDestroyAnimator  = Safe.destroyAnimator
local SafeChangeState      = Safe.changeState
local SafeSetHumanoidProp  = Safe.setHumanoidProp
local tabHome = ui:CreateSection("Home")
local tabFarm = ui:CreateSection("Auto Farm")
local tabItem = ui:CreateSection("Item Quest")
local tabStats = ui:CreateSection("Stats")
local tabEsp = ui:CreateSection("ESP")
local tabRaid = ui:CreateSection("Raid")
local tabPlayer = ui:CreateSection("Players")
local tabTeleport = ui:CreateSection("Teleport")
local tabStatus = ui:CreateSection("Status")
local tabDevil = ui:CreateSection("Devil Fruit")
local tabRace = ui:CreateSection("Race V4")
local tabShop = ui:CreateSection("Shop")
local tabMisc = ui:CreateSection("Misc")
...
local SafeSetHeadCanCollide = Safe.setHeadCanCollide
local SafeGetHRP            = Safe.getHRP
local SafeGetCombatFramework= Safe.getCombatFramework
        ...
        if not isnil(v.Character) then
            if ESPPlayer then
                if not isnil(v.Character.Head) and not v.Character.Head:FindFirstChild('NameEsp'..Number) then
                    ...
                    v.Character.Head['NameEsp'..Number].TextLabel.Text = (v.Name ..' | '.. round((game:GetService('Players').LocalPlayer.Character.Head.Position - v.Character.Head.Position).Magnitude/3) ..' Distance\nHealth : ' .. round((v.Character and v.Character:FindFirstChild('Humanoid') and v.Character.Humanoid.Health or 0)*100/(v.Character and v.Character:FindFirstChild('Humanoid') and v.Character.Humanoid.MaxHealth or 1)) .. '%')
                end
            else
...
-- (Duplicate ESP block removed — UpdatePlayerChams/UpdateChestChams/UpdateDevilChams/
--  UpdateFlowerChams/UpdateRealFruitChams defined once in the first ESP block above)
...
function BTP(P)
	repeat task.wait(1)
		pcall(function() game.Players.LocalPlayer.Character.Humanoid:ChangeState(15) end)
        ...
        pcall(function()
            if _G.AutoFarmNearest and AutoFarmNearestMagnet or SelectMag and _G.BringMonster then
                for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
                    if not string.find(v.Name,"Boss") and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= _G.BringMode then
                        if InMyNetWork(v.HumanoidRootPart) then
                            v.HumanoidRootPart.CFrame = PosMon
                            SafeSetJumpPower(v, 0)
                            SafeSetWalkSpeed(v, 0)
                            v.HumanoidRootPart.Size = Vector3.new(60,60,60)
                            v.HumanoidRootPart.Transparency = 1
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                            SafeDestroyAnimator(v)
                            SafeChangeState(v, 11)
                            SafeChangeState(v, 14)
                        end
                    end
                end
            end
        ...
        while task.wait() do
            pcall(function()
                if STOPRL then
                    ...
                    local Hits = STOPRL and STOPRL.getBladeHits and STOPRL.getBladeHits(b,c,d) or nil
                    if Hits then
                        if _G.FastAttack then
                            if STOP then STOP.play = function() end end
                            a:Play(0.01,0.01,0.01)
                            func(Hits)
                            if STOP and shared.cpc then STOP.play = shared.cpc end
...
function GetBladeHit()
    local CombatFrameworkLib = SafeGetCombatFramework()
    local CmrFwLib = CombatFrameworkLib and (CombatFrameworkLib and CombatFrameworkLib[2]) or nil
    if not CmrFwLib or not CmrFwLib.activeController then return nil end
    local p13 = CmrFwLib.activeController
    local weapon = p13.blades and p13.blades[1]
    if not weapon then
        return weapon
    end
...
function AttackHit()
    local CombatFrameworkLib = SafeGetCombatFramework()
    local CmrFwLib = CombatFrameworkLib and (CombatFrameworkLib and CombatFrameworkLib[2]) or nil
    if not CmrFwLib or not CmrFwLib.activeController then return end
    local plr = game.Players.LocalPlayer
    for i = 1, 1 do
        local _bhOk, bladehit = pcall(function() return require(game.ReplicatedStorage.CombatFramework.RigLib).getBladeHits(plr.Character,{plr.Character.HumanoidRootPart},60) end)
        if not _bhOk then bladehit = nil end
        if bladehit and type(bladehit) == "table" then
            local cac = {}
            local hash = {}
            ...
            if #bladehit > 0 then
                pcall(function()
                    ...
                    local bladeName = GetBladeHit()
                    if bladeName then
                        game:GetService("ReplicatedStorage").RigControllerEvent:FireServer("weaponChange",tostring(bladeName))
                    end
...
tabHome:createLabel({ Name = "[+] Sla Hub is HERE!" })
tabHome:createLabel({ Name = "[+] Best AutoFarm!" })
tabHome:createLabel({ Name = "[+] Up and Growing" })
tabHome:createLabel({ Name = "[+] Smooth" })

    Name = "Status"
})

local locallv = tabHome:createLabel({ Name = "Level" })
    ...
    local localrace = tabHome:createLabel({ Name = "Race" })
    ...
    local localbeli = tabHome:createLabel({ Name = "Beli" })
    ...
    local localfrag = tabHome:createLabel({ Name = "Fragment" })
    ...
    local localexp = tabHome:createLabel({ Name = "ExP" })
    ...
    local localstat = tabHome:createLabel({ Name = "Stats Points" })
    ...
    local localbountyhornor = tabHome:createLabel({ Name = "Bounty" })
    ...
    local localDevil = tabHome:createLabel({ Name = "Devil Fruit" })
    ...
    local Section = tabHome:createLabel({ Special = true,
        Name = "Status Hack"
    })

    local LevelFarmFarm = tabHome:createLabel({ Name = "Kaitun: Coming Soon" })
    tabHome:createLabel({ Name = "Config Save : Comming Son..." })
    local Farmfast = tabHome:createLabel({ Name = "Farm Fast : ❌" })
    local Bypass = tabHome:createLabel({ Name = "Bypass ❌" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "World"
    })
    local WolrdSet3 = tabHome:createLabel({ Name = "World 1 ❌ " })
    local WolrdSet = tabHome:createLabel({ Name = "Wolrd : 2 ❌" })
    local WolrdSet1 = tabHome:createLabel({ Name = "Wolrd : 3 ❌" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "Stats Point"
    })
    local StastMelee = tabHome:createLabel({ Name = "" })
    local StastDefense = tabHome:createLabel({ Name = "" })
    local StastSword = tabHome:createLabel({ Name = "" })
    local StastGun = tabHome:createLabel({ Name = "" })
    local StastDevilFruit = tabHome:createLabel({ Name = "" })
    local PointsStast = tabHome:createLabel({ Name = "" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "Melee Check"
    })
    local Superhuman = tabHome:createLabel({ Name = "❌ : Superhuman" })
    local DeathStep = tabHome:createLabel({ Name = "❌ : Sharkman Karate" })
    local ElectricClaw = tabHome:createLabel({ Name = "❌ : Electric Claw" })
    local DragonTalon = tabHome:createLabel({ Name = "❌ : Dragon Talon" })
    local GodHuman = tabHome:createLabel({ Name = "❌ : God Human" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "Sword Legend Check"
    })
    local Shisui = tabHome:createLabel({ Name = "❌ : Shisui" })
    local Saddi = tabHome:createLabel({ Name = "❌ : Saddi" })
    local Wando = tabHome:createLabel({ Name = "❌ : Wando" })
    local TrueTripleKatana = tabHome:createLabel({ Name = "❌ : True Triple Katana" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "World 1 Weapon"
    })
    local Saber = tabHome:createLabel({ Name = "❌ : Saber" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "World 2 Weapon"
    })
    local Rengoku = tabHome:createLabel({ Name = "❌ : Rengoku" })
    local MidnightBlade = tabHome:createLabel({ Name = "❌ : Midnight Blade" })
    local DragonTrident = tabHome:createLabel({ Name = "❌ : DragonTrident" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "World 3 Weapon"
    })
    local Yama = tabHome:createLabel({ Name = "❌ : Yama" })
    local BuddySword = tabHome:createLabel({ Name = "❌ : Buddy Sword" })
    local Canvander = tabHome:createLabel({ Name = "❌ : Canvander" })
    local TwinHooks = tabHome:createLabel({ Name = "❌ : Twin Hooks" })
    local SpikeyTrident = tabHome:createLabel({ Name = "❌ : Spikey Trident" })
    local HallowScythe = tabHome:createLabel({ Name = "❌ : Hallow Scythe" })
    local DarkDagger = tabHome:createLabel({ Name = "❌ : Dark Dagger" })
    local Tushita = tabHome:createLabel({ Name = "❌ : Tushita" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "Gun"
    })
    local Kabucha = tabHome:createLabel({ Name = "❌ : Kabucha" })
    local AcidumRifle = tabHome:createLabel({ Name = "❌ : Acidum Rifle" })
    local BizarreRifle = tabHome:createLabel({ Name = "❌ : Bizarre Rifle" })
    local Section = tabHome:createLabel({ Special = true,
        Name = "Quest"
    })
    local BartiloQuest = tabHome:createLabel({ Name = "❌ : Bartilo Quest" })
    local DonSwanQuest = tabHome:createLabel({ Name = "❌ : Don Swan Quest" })
    local KillDonSwan = tabHome:createLabel({ Name = "❌ : Kill Don Swan" })

    Name = "Acessory"
})

local Dark_Coat = tabHome:createLabel({ Name = "❌: Dark Coat" })
local Ghoul_Mask = tabHome:createLabel({ Name = "❌: Ghoul Mask" })
local Swan_Glass = tabHome:createLabel({ Name = "❌: Swan Glass" })
local Pale_Scarf = tabHome:createLabel({ Name = "❌: Pale Scarf" })
local Valkyrie_Helm = tabHome:createLabel({ Name = "❌: Valkyrie Helm" })
...
tabFarm:createLabel({ Name = "Select Weapon","Please Select Weapon")
...
if World2 and World1 then
    local Section = tabFarm:createLabel({ Special = true,
        Name = "BOOTS"
    })
    ...
    tabFarm:createLabel({ Name = "Redeem Code","Click to Button to Reddem All Code In Blox Fruits.")
...
tabFarm:createLabel({ Name = "Main Farm","Click to Box to Farm, I ready update new mob farm!.")

ToggleFarm = tabFarm:createToggle({
    Name = "Auto Farm",
...
tabFarm:createLabel({ Name = "Mastery Menu","Click To Box to Start Farm Mastery")
...
Farmfasttoggle = tabFarm:createToggle({
    Name = "Auto Farm Fast",
...
tabFarm:createLabel({ Name = "Kaitun/Cantay","Kaitun bản này đell khác gì bản kia.")
...
if World1 then
    tabFarm:createDropdown({
        ...
        List = {"Bandit","Monkey","Gorilla","Pirate","Brute","Desert Bandit","Desert Officer","Snow Bandit","Snowman","Chief Petty Officer","Sky Bandit","Dark Master","Prisoner", "Dangerous Prisoner","Toga Warrior","Gladiator","Military Soldier","Military Spy","Fishman Warrior","Fishman Commando","God's Guard","Shanda","Royal Squad","Royal Soldier","Galley Pirate ","Galley Captain"},
        Callback = function(Value)
            SelectMonster = Value
        end
    })
    ...
    tabFarm:createDropdown({
        ...
        List = {"Raider","Mercenary","Swan Pirate","Factory Staff","Marine Lieutenant","Marine Captain","Zombie","Vampire","Snow Trooper","Winter Warrior","Lab Subordinate","Horned Warrior","Magma Ninja","Lava Pirate","Ship Deckhand","Ship Engineer","Ship Steward","Ship Officer","Arctic Warrior","Snow Lurker","Sea Soldier","Water Fighter"},
        Callback = function(Value)
            SelectMonster = Value
        end
    })
    ...
    tabFarm:createDropdown({
        ...
        List = {"Pirate Millionaire","Dragon Crew Warrior","Dragon Crew Archer","Female Islander","Giant Islander","Marine Commodore","Marine Rear Admiral","Fishman Raider","Fishman Captain","Forest Pirate","Mythological Pirate","Jungle Pirate","Musketeer Pirate","Reborn Skeleton","Living Zombie","Demonic Soul","Posessed Mummy", "Peanut Scout", "Peanut President", "Ice Cream Chef", "Ice Cream Commander", "Cookie Crafter", "Cake Guard", "Baking Staff", "Head Baker", "Cocoa Warrior", "Chocolate Bar Battler", "Sweet Thief", "Candy Rebel", "Candy Pirate", "Snow Demon"},
        Callback = function(Value)
            SelectMonster = Value
        end
    })
...
TweenChest = tabFarm:createToggle({
        Name = "Auto Chest [MUP]",
...
tabFarm:createToggle({
    ...
    flagName = "Stop Only Dark Key or God's Chalice",
...
spawn(function()
    ...
    if _G.StopChest then
        if game.Players.LocalPlayer.Backpack:FindFirstChild("Fist of Darkness") or game.Players.LocalPlayer.Character:FindFirstChild("Fist of Darkness") or game.Players.LocalPlayer.Backpack:FindFirstChild("God's Chalice") or game.Players.LocalPlayer.Character:FindFirstChild("God's Chalice") then
            AutoFarmChest = false
            StopTween(AutoFarmChest)
            Library.Flags["TweenChest"] = false
        end
    end
end
end)
...
local Bone = tabFarm:createLabel({ Name = "Bone : " })
...
ToggleBone = tabFarm:createToggle({
    Name = "Auto Farm Bone",

    flagName = "Auto Farm Bone",
...
ToggleRandom = tabFarm:createToggle({
        Name = "Auto Random Bone",

        flagName = "Auto Random Bone",
...
local EliteProgress = tabFarm:createLabel({ Name = "" })
...
tabFarm:createLabel({ Name = "Auto Farm Boss :","Chờ Update thôi ae!!!")
    ...
    tabFarm:createToggle({
        ...
        flagName = "Auto Farm Observation Haki",
    ...
    tabFarm:createToggle({
        ...
        flagName = "Auto Farm Observation Haki[HOP]",
...
local SaberBoss = tabItem:createLabel({ Name = "Boss Saber : " })
spawn(function()
    while task.wait() do
        pcall(function()
...
local PoleBoss = tabItem:createLabel({ Name = "Boss Thunder God : " })
spawn(function()
    while task.wait() do
        pcall(function()
...
TogglePolev1 = tabItem:createToggle({
    Name = "Auto Pole V.1",
...
TogglePolev1Hop = tabItem:createToggle({
    Name = "Auto Pole V.1[HOP]",

    flagName = "Auto Pole[HOP]",
...
BartiloToggle = tabItem:createToggle({
    Name = "Auto Bartilo Quest",

    flagName = "Auto Bartilo",
...
tabItem:createToggle({
    ...
    local Section = tabItem:createLabel({ Special = true,
        Name = "Soul Guita"
    })
...
tabItem:createToggle({
    ...
    flagName = "Auto DualCursetKatana",
...
tabItem:createToggle({
    ...
    Callback = function(Value)
        ...
        tabItem:createLabel({ Name = "Cho thằng nào đell hiểu thì đây là tự động đánh hình nộm" })
...
ToggleHallow = tabItem:createToggle({
            Name = "Auto Hallow Scythe[HOP]",

            flagName = "Auto Hallow[HOP]",
        ...
        pcall(function()
            for i,v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                if v:IsA('Tool') and not (v.Name == "Summon Sea Beast" or v.Name == "Water Body" or v.Name == "Awakening") then
                    ...
                    pcall(function() game.Players.LocalPlayer.Character.Humanoid:EquipTool(ToolHumanoid) end)
                    task.wait(1)
                end
            end
        end)
...
Chest = tabStats:createLabel({ Name = "Chest" })

Fruit = tabStats:createLabel({ Name = "Fruit" })

  local Pointstat = tabStats:createLabel({ Name = "Stat Points" })
...
local Melee = tabStats:createLabel({ Name = "Melee : " })
local Defense = tabStats:createLabel({ Name = "Defense : " })
local Sword = tabStats:createLabel({ Name = "Sword : " })
local Gun = tabStats:createLabel({ Name = "Gun : " })
local Fruit = tabStats:createLabel({ Name = "Fruit : " })
...
-- (isnil/round/Number/UpdatePlayerChams/UpdateDevilChams defined once at top / first ESP block)
...
local TimeRaid = tabRaid:createLabel({ Name = "Wait For Dungeon" })

local checkisland = tabRaid:createLabel({ Name = "Island : Not Raid" })

_G.SelectChip = selectraids or ""
    Raidslist = {}
    RaidsModule = pcall(function() return require(game.ReplicatedStorage.Raids) end) and require(game.ReplicatedStorage.Raids) or nil
    for i,v in pairs(RaidsModule and RaidsModule.raids or {}) do
        table.insert(Raidslist,v)
    end
    for i,v in pairs(RaidsModule and RaidsModule.advancedRaids or {}) do
        table.insert(Raidslist,v)
    end
...
if World2 then
    ...
    if World3 then
        ...
        local Section = tabPlayer:createLabel({ Special = true,
...
local SelectedPly = tabPlayer:createDropdown({
    Name = "Select Players",
...
tabPlayer:createButton({
    ...
    SelectedPly:updateList(NewPlayerList)
      end
})
    ...
    Callback = function(Value)
        ...
        pcall(function()
            if _G.TeleportPly then
                repeat topos(game:GetService("Players")[_G.SelectPly].Character.HumanoidRootPart.CFrame) task.wait() until _G.TeleportPly == false
            end
...
local FM = tabStatus:createLabel({ Name = "Server Time : " .. function8() .. " | ".. CheckMoon() .. " | " .. function7())

spawn(function()
        while task.wait() do
            tabStatus:Set("Server Time : " .. function8() .. " | ".. CheckMoon() .. " | " .. function7())
                end
            end)

            local Section = tabStatus:createLabel({ Special = true,
                Name = "Anclient One Check"
            })

            local bL = tabStatus:createLabel({ Name = "Anclient One Status : " .. tostring(CheckAcientOneStatus()))

    Name = "Elite Check"
})

local Elite_Hunter_Status = tabStatus:createLabel({ Name = "Only Third Sea" })
        ...
        local EliteProgress = tabStatus:createLabel({ Name = "" })
...
Mirragecheck = tabStatus:createLabel({ Name = "Only Third Sea" })

    Name = "Dough King Check"
})

local MobKilled = tabStatus:createLabel({ Name = "Only Third Sea" })
...
tabDevil:createToggle({
    Name = "Auto Random Fruits",

    flagName = "Auto Random Fruits",
...
tabDevil:createToggle({
    ...
    flagName = "Teleport To Fruit Spawn",
...
tabDevil:createToggle({
    ...
    flagName = "Bring All Fruit[75% Kick]",
...
AutoPullLever = tabRace:createToggle({
    Name = "Auto Pull Lever",

    flagName = "Auto Pull Lever",
...
MoveCamTooMoonToggle = tabRace:createToggle({
    Name = "Move Cam to Moon",

    flagName = "Move Cam to Moon",
...
spawn(function()
    ...
    local Section = tabMisc:createLabel({ Special = true,
        Name = "Rejoin"
    })
            ...
            pcall(function()
                ...
                elseif game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                    ...
                    if game:GetService("Workspace").Enemies:FindFirstChild(Mon) then
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and SafeHumanoidHealth(v) > 0 then
                                if v.Name == Mon then
                                    if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
                                        repeat task.wait()
                                            ...
                                            game:GetService'VirtualUser':CaptureController()
                                            game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
                                        until not _G.AutoFarm or SafeHumanoidHealth(v) <= 0 or not v.Parent or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                    else
                                        StartMagnet = false
                                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
                                    end
                    ...
                    if game:GetService("Workspace").Enemies:FindFirstChild(Mon) then
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and SafeHumanoidHealth(v) > 0 then
                                if v.Name == Mon then
                                    if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
                                        HealthMs = SafeHumanoidMaxHealth(v) * _G.Kill_At/100
                                        repeat task.wait()
                                            if SafeHumanoidHealth(v) <= HealthMs then
                                                AutoHaki()
                                            ...
                                            game:GetService'VirtualUser':CaptureController()
                                            game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
                                        until not _G.AutoFarmFruitMastery or SafeHumanoidHealth(v) <= 0 or not v.Parent or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                    else
                                        UseSkill = false
                                        StartMasteryFruitMagnet = false
                                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
                        ...
                        if Mob then
                            ...
                            if game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame.Y <= 1 then
                                pcall(function() game:GetService("Players").LocalPlayer.Character.Humanoid.Jump = true end)
                                task.wait()
                                pcall(function() game:GetService("Players").LocalPlayer.Character.Humanoid.Jump = false end)
                            end
                        end
                    end
                end
                        ...
                        pcall(function()
                            for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v.Name == Mon then
                                    repeat task.wait()
                                        if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, NameMon) then
                                            HealthMin = SafeHumanoidMaxHealth(v) * _G.Kill_At/100
                                            if SafeHumanoidHealth(v) <= HealthMin thenEquipWeapon(SelectWeaponGun)
                                    ...
                                    until SafeHumanoidHealth(v) <= 0 or _G.AutoFarmGunMastery == false or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                    StartMasteryGunMagnet = false
                                end
                            end
                        end)
            ...
            pcall(function()
                if game.Players.LocalPlayer.Data.Level.Value >= 700 and World1 then
                    ...
                    if game.Workspace.Map.Ice.Door.CanCollide == true and game.Workspace.Map.Ice.Door.Transparency == 0 then
                        repeat task.wait() topos(CFrame.new(4851.8720703125, 5.6514348983765, 718.47094726563)) until (CFrame.new(4851.8720703125, 5.6514348983765, 718.47094726563).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea
                        task.wait(1)
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("DressrosaQuestProgress","Detective")
                        EquipWeapon("Key")
                        local pos2 = CFrame.new(1347.7124, 37.3751602, -1325.6488)
                        repeat task.wait() topos(pos2) until (pos2.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoSecondSea
                        task.wait(3)
                    elseif game.Workspace.Map.Ice.Door.CanCollide == false and game.Workspace.Map.Ice.Door.Transparency == 1 then
                        if game:GetService("Workspace").Enemies:FindFirstChild("Ice Admiral") then
                            for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v.Name == "Ice Admiral" and SafeHumanoidHealth(v) > 0 then
                                    repeat task.wait()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                        v.HumanoidRootPart.CanCollide = false
                                    ...
                                    until SafeHumanoidHealth(v) <= 0 or not v.Parent or not _G.AutoSecondSea
                                    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelDressrosa")
                                end
                            end
                        else
    ...
    while task.wait() do
        pcall(function()
            if _G.BringMonster then
                CheckQuest()
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if _G.AutoFarm and StartMagnet and v.Name == Mon and (Mon == "Factory Staff" or Mon == "Monkey" or Mon == "Dragon Crew Warrior" or Mon == "Dragon Crew Archer") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 and (v.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 220 then
                        v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                        v.HumanoidRootPart.CFrame = PosMon
                        SafeChangeState(v, 14)
                        v.HumanoidRootPart.CanCollide = false
                    ...
                    elseif _G.AutoFarm and StartMagnet and v.Name == Mon and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 and (v.HumanoidRootPart.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= _G.BringMode then
                        v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                        v.HumanoidRootPart.CFrame = PosMon
                        SafeChangeState(v, 14)
                        v.HumanoidRootPart.CanCollide = false
            ...
            if _G.BringMonster then
                ...
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    ...
                    if _G.Farmfast and StardMag then
                        if (v.Name == "Shanda") and (v.HumanoidRootPart.Position - FastMon.Position).Magnitude <= 250 and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                    ...
                    if _G.AutoVampire and YouSkidder then
                        if (v.Name == "Vampire") and (v.HumanoidRootPart.Position - PosVui.Position).Magnitude <= 500 and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                    ...
                    if _G.AutoEctoplasm and StartEctoplasmMagnet then
                        if string.find(v.Name, "Ship") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 and (v.HumanoidRootPart.Position - EctoplasmMon.Position).Magnitude <= _G.BringMode then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                    ...
                    if _G.AutoRengoku and StartRengokuMagnet then
                        if (v.Name == "Snow Lurker" or v.Name == "Arctic Warrior") and (v.HumanoidRootPart.Position - RengokuMon.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                    ...
                    if _G.AutoMusketeerHat and StartMagnetMusketeerhat then
                        if v.Name == "Forest Pirate" and (v.HumanoidRootPart.Position - MusketeerHatMon.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            SafeChangeState(v, 14)
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.Auto_EvoRace and StartEvoMagnet then
                        if v.Name == "Zombie" and (v.HumanoidRootPart.Position - PosMonEvo.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            SafeChangeState(v, 14)
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.AutoBartilo and AutoBartiloBring then
                        if v.Name == "Swan Pirate" and (v.HumanoidRootPart.Position - PosMonBarto.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            SafeChangeState(v, 14)
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.AutoFarmFruitMastery and StartMasteryFruitMagnet then
                        if v.Name == "Monkey" then
                            if (v.HumanoidRootPart.Position - PosMonMasteryFruit.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                                SafeChangeState(v, 14)
                                v.HumanoidRootPart.CanCollide = false
                                SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.AutoFarmGunMastery and StartMasteryGunMagnet then
                        if v.Name == "Monkey" then
                            if (v.HumanoidRootPart.Position - PosMonMasteryGun.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                                SafeChangeState(v, 14)
                                v.HumanoidRootPart.CanCollide = false
                                SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.Auto_Bone and StartMagnetBoneMon then
                        if (v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or v.Name == "Demonic Soul" or v.Name == "Posessed Mummy") and (v.HumanoidRootPart.Position - PosMonBone.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            SafeChangeState(v, 14)
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.AutoFarmCandy and StartCandyMagnet then
                        if (v.Name == "Ice Cream Chef" or v.Name == "Ice Cream Commander") and (v.HumanoidRootPart.Position - CandyMon.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            SafeChangeState(v, 14)
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                    ...
                    if _G.AutoDoughtBoss and MagnetDought then
                        if (v.Name == "Cookie Crafter" or v.Name == "Cake Guard" or v.Name == "Baking Staff" or v.Name == "Head Baker") and (v.HumanoidRootPart.Position - PosMonDoughtOpenDoor.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            SafeChangeState(v, 14)
                            v.HumanoidRootPart.CanCollide = false
                            SafeSetHeadCanCollide(v, false)
                        ...
                        if StardFarm and FarmMag then
                            if (v.Name == "Sweet Thief" or v.Name == "Candy Rebel" or v.Name == "Cocoa Warrior" or v.Name == "Chocolate Bar Battler") and (v.HumanoidRootPart.Position - PosMonDoughtOpenDoor.Position).Magnitude <= _G.BringMode and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                v.HumanoidRootPart.Size = Vector3.new(50,50,50)
                                SafeChangeState(v, 14)
                                v.HumanoidRootPart.CanCollide = false
                                SafeSetHeadCanCollide(v, false)
        ...
        while task.wait() do
            if _G.Farmfast and World1 then
                if game.Players.LocalPlayer.Data.Level.Value >= 10 then
                    Library.Flags["ToggleFarm"] = false
                    _G.Farmfast = true
                end
            end
        end
                ...
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(-7894.6176757813, 5547.1416015625, -380.29119873047))
                    for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if v.Name == "Shanda" then
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                repeat task.wait()
                                    AutoHaki()
                                    EquipWeapon(_G.SelectWeapon)
                                    v.HumanoidRootPart.CanCollide = false
                                    ...
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                until not _G.Farmfast or not v.Parent or SafeHumanoidHealth(v) <= 0
                                StardMag = false
        ...
        while task.wait() do
            if _G.Farmfast and World1 then
                if game.Players.LocalPlayer.Data.Level.Value >= 50 then
                    Library.Flags["Farmfasttoggle"] = false
...
spawn(function()
    ...
    pcall(function()
     ...
     if SafeHumanoidHealth(v) > 0 then
     ...
     until not _G.AutoFarmSelectMonster or not v.Parent or SafeHumanoidHealth(v) == 0 or not game:GetService("Workspace").Enemies:FindFirstChild(v.Name)
     end
     end
     end
     end
                        ...
                        repeat task.wait()
                          EquipWeapon(_G.SelectWeapon)
                            ...
                            game:GetService("VirtualUser"):CaptureController()
                               ...
                               PosMon = v.HumanoidRootPart.CFrame
                        until not _G.AutoFarmNearest or not v.Parent or SafeHumanoidHealth(v) <= 0
                        AutoFarmNearestMagnet = false
                        Fastattack = false
                    end
                end
    ...
    spawn(function()
        ...
        spawn(function()
            ...
            pcall(function()
             ...
             for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
             if v.Name == "Core" and SafeHumanoidHealth(v) > 0 then
             repeat task.wait()
             AutoHaki()
             EquipWeapon(_G.SelectWeapon)
             ...
             game:GetService("VirtualUser"):CaptureController()
             game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
             until SafeHumanoidHealth(v) <= 0 or _G.AutoFactory == false
             end
             end
             else
              topos(CFrame.new(448.46756, 199.356781, -441.389252))
                    ...
                    if _G.RaidPirate then
                        pcall(function()
                            ...
                            if (CFrame.new(-5539.3115234375, 313.800537109375, -2972.372314453125).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 500 then
                                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                    if _G.RaidPirate and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and SafeHumanoidHealth(v) > 0 then
                                        if (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 2000 then
                                            repeat task.wait()
                                                ...
                                                topos(v.HumanoidRootPart.CFrame * Pos)
                                                Click()
                                            until SafeHumanoidHealth(v) <= 0 or not v.Parent or not _G.RaidPirate
                                        end
                                    end
                                end
                            else
                        ...
                        if _G.Kill_Aura then
                            for i,v in pairs(game.Workspace.Enemies:GetDescendants()) do
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                    pcall(function()
                                        repeat task.wait(.1)
                                            SafeSetHumanoidProp(v, "Health", 0)
                                            v.HumanoidRootPart.CanCollide = false
                                            sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                        until not _G.Kill_Aura  or not v.Parent or SafeHumanoidHealth(v) <= 0
                                    end)
                                end
                            end
                        end
                        ...
                        if _G.Auto_Kill_Law then
                            pcall(function()
                                if game:GetService("ReplicatedStorage"):FindFirstChild("Order") or game:GetService("Workspace").Enemies:FindFirstChild("Order") then
                                    for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
                                        if _G.Auto_Kill_Law and v.Name == "Order" and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and SafeHumanoidHealth(v) > 0 then
                                            repeat task.wait()
                                                AutoHaki()
                                                EquipWeapon(_G.SelectWeapon)
                                                v.HumanoidRootPart.CanCollide = false
                                                ...
                                                game:GetService'VirtualUser':CaptureController()
                                                game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
                                            until not _G.Auto_Kill_Law or SafeHumanoidHealth(v) <= 0 or not v.Parent
                                        end
                                    end
                                end
                            end)
                        ...
                        if v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or v.Name == "Demonic Soul" or v.Name == "Posessed Mummy" then
                           if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                               repeat task.wait()
                                    ...
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                until not _G.Auto_Bone or not v.Parent or SafeHumanoidHealth(v) <= 0
                            end
                        end
                    end
                else
                                ...
                                if v.Name == "Diablo" or v.Name == "Deandre" or v.Name == "Urban" then
                                    if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                        repeat task.wait()
                                            ...
                                            game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                            sethiddenproperty(game:GetService("Players").LocalPlayer,"SimulationRadius",math.huge)
                                        until _G.AutoElitehunter == false or SafeHumanoidHealth(v) <= 0 or not v.Parent
                                    end
                                end
                            end
                        else
                        ...
                        if v.Name == "Cake Prince" then
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                repeat task.wait()
                                    ...
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                    sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                until not _G.AutoDoughtBoss or not v.Parent or SafeHumanoidHealth(v) <= 0
                            end
                        end
                    end
                else
                                    ...
                                    if v.Name == "Cookie Crafter" or v.Name == "Cake Guard" or v.Name == "Baking Staff" or v.Name == "Head Baker" then
                                        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                            repeat task.wait()
                                                ...
                                                game:GetService("VirtualUser"):CaptureController()
                                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                            until not _G.AutoDoughtBoss or not v.Parent or SafeHumanoidHealth(v) <= 0 or game:GetService("Workspace").Map.CakeLoaf.BigMirror.Other.Transparency == 0 or game:GetService("ReplicatedStorage"):FindFirstChild("Cake Prince") or game:GetService("Workspace").Enemies:FindFirstChild("Cake Prince") or KillMob == 0
                                        end
                                    end
                                end
                            else
                        ...
                        if v.Name == "Dough King" then
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                repeat task.wait()
                                    ...
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                    sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                until not  _G.Autodoughking or not v.Parent or SafeHumanoidHealth(v) <= 0
                            end
                        end
                    end
                else
                            ...
                            if v.Name == "Vampire" then
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                    repeat task.wait()
                                        ...
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                    until not _G.AutoVampire or not v.Parent or SafeHumanoidHealth(v) <= 0
                                    YouSkidder = false
                                end
                            end
                        end
            ...
            pcall(function()
                ...
                if string.find(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuySharkmanKarate"), "keys") then
                    if game:GetService("Players").LocalPlayer.Character:FindFirstChild("Water Key") or game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Water Key") then
                        repeat task.wait() getgenv().ToTarget(-2604.6958, 239.432526, -10315.1982, 0.0425701365, 0, -0.999093413, 0, 1, 0, 0.999093413, 0, 0.0425701365) until (CFrame.new(-2604.6958, 239.432526, -10315.1982, 0.0425701365, 0, -0.999093413, 0, 1, 0, 0.999093413, 0, 0.0425701365).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.Auto_Fully_SharkMan_Karate
                                ...
                                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                    if v.Name == "Tide Keeper" then
                                        repeat task.wait()
                                            ...
                                            game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
                                            sethiddenproperty(game:GetService("Players").LocalPlayer,"SimulationRadius",math.huge)
                                        until not v.Parent or SafeHumanoidHealth(v) <= 0 or _G.AutoSharkman == false or game:GetService("Players").LocalPlayer.Character:FindFirstChild("Water Key") or game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Water Key")
                                    end
                                end
                            else
                                repeat task.wait() getgenv().ToTarget(game:GetService("ReplicatedStorage"):FindFirstChild("Tide Keeper").HumanoidRootPart.CFrame) until game:GetService("Workspace").Enemies:FindFirstChild("Tide Keeper")
                            end
                        else
                            Hop()
                        end
                                            ...
                                            if v.Name == "Mob Leader" then
                                               if game:GetService("Workspace").Enemies:FindFirstChild("Mob Leader") then
                                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                                    ...
                                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                                    sethiddenproperty(game:GetService("Players").LocalPlayer,"SimulationRadius",math.huge)
                                                    until SafeHumanoidHealth(v) <= 0 or not _G.Auto_Saber
                                                 end
                                            end
                    ...
                    if v.Name == "Thunder God" then
                        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            repeat task.wait()
                                ...
                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                            until not  _G.Autopole or not v.Parent or SafeHumanoidHealth(v) <= 0
                        end
                    end
                end
            else
            ...
            if _G.AutoBartilo then
                            ...
                            for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v.Name == Ms then
                                    pcall(function()
                                        ...
                                        until not v.Parent or SafeHumanoidHealth(v) <= 0 or _G.AutoBartilo == false or game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false
                                        AutoBartiloBring = false
                                    end)
                                end
                            end
                        else
                            repeat topos(CFrame.new(932.624451, 156.106079, 1180.27466, -0.973085582, 4.55137119e-08, -0.230443969, 2.67024713e-08, 1, 8.47491108e-08, 0.230443969, 7.63147128e-08, -0.973085582)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(932.624451, 156.106079, 1180.27466, -0.973085582, 4.55137119e-08, -0.230443969, 2.67024713e-08, 1, 8.47491108e-08, 0.230443969, 7.63147128e-08, -0.973085582)).Magnitude <= 10
                        end
                    else
                        repeat topos(CFrame.new(-456.28952, 73.0200958, 299.895966)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-456.28952, 73.0200958, 299.895966)).Magnitude <= 10
                        task.wait(1.1)
                ...
                elseif game:GetService("Players").LocalPlayer.Data.Level.Value >= 800 and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 1 then
                        ...
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v.Name == Ms then
                                ...
                                repeat task.wait()
                                    ...
                                    game:GetService'VirtualUser':Button1Down(Vector2.new(1280, 672))
                                    sethiddenproperty(game:GetService("Players").LocalPlayer,"SimulationRadius",math.huge)
                                until not v.Parent or SafeHumanoidHealth(v) <= 0 or _G.AutoBartilo == false
                            end
                        end
                    elseif game:GetService("ReplicatedStorage"):FindFirstChild("Jeremy") then
                        ...
                        task.wait(1)
                        repeat topos(CFrame.new(2099.88159, 448.931, 648.997375)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(2099.88159, 448.931, 648.997375)).Magnitude <= 10
                        task.wait(2)
                    else
                        repeat topos(CFrame.new(2099.88159, 448.931, 648.997375)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(2099.88159, 448.931, 648.997375)).Magnitude <= 10
                    end
                elseif game:GetService("Players").LocalPlayer.Data.Level.Value >= 800 and game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 2 then
                    repeat topos(CFrame.new(-1850.49329, 13.1789551, 1750.89685)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1850.49329, 13.1789551, 1750.89685)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1858.87305, 19.3777466, 1712.01807)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1858.87305, 19.3777466, 1712.01807)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1803.94324, 16.5789185, 1750.89685)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1803.94324, 16.5789185, 1750.89685)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1858.55835, 16.8604317, 1724.79541)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1858.55835, 16.8604317, 1724.79541)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1869.54224, 15.987854, 1681.00659)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1869.54224, 15.987854, 1681.00659)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1800.0979, 16.4978027, 1684.52368)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1800.0979, 16.4978027, 1684.52368)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1819.26343, 14.795166, 1717.90625)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1819.26343, 14.795166, 1717.90625)).Magnitude <= 10
                    task.wait(1)
                    repeat topos(CFrame.new(-1813.51843, 14.8604736, 1724.79541)) task.wait() until not _G.AutoBartilo or (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position-Vector3.new(-1813.51843, 14.8604736, 1724.79541)).Magnitude <= 10
                end
            end
        end
    end)
        ...
        while task.wait() do
            if _G.AutoBartilo then
                if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BartiloQuestProgress","Bartilo") == 3 then
                    Library.Flags["BartiloToggle"] = false
                    ToggleFarm:Set(true)
                end
            end
        end
                                ...
                                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                    if v.Name == "rip_indra" then
                                        ...
                                        repeat task.wait()
                                            ...
                                            game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("TravelZou")
                                            sethiddenproperty(game:GetService("Players").LocalPlayer,"SimulationRadius",math.huge)
                                        until _G.AutoThirdSea == false or SafeHumanoidHealth(v) <= 0 or not v.Parent
                                    end
                                end
                            ...
                            if v.Name == "Cake Queen" then
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                    repeat task.wait()
                                        ...
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                        sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                    until not _G.AutoBudySword or not v.Parent or SafeHumanoidHealth(v) <= 0
                                end
                            end
                        end
                    else
                            ...
                            if game:GetService("Workspace").NPCs:FindFirstChild("Skeleton Machine") then
                                ...
                                if game:GetService("Workspace").Map["Haunted Castle"].Candle1.Transparency == 0 then
                                    if game:GetService("Workspace").Map["Haunted Castle"].Placard1.Left.Part.Transparency == 0 then
                                        Quest2 = true
                                        repeat task.wait() topos(CFrame.new(-8762.69140625, 176.84783935546875, 6171.3076171875)) until (CFrame.new(-8762.69140625, 176.84783935546875, 6171.3076171875).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoNevaSoulGuitar
                                        task.wait(1)
                                    ...
                                    elseif game:GetService("Workspace").Map["Haunted Castle"].Tablet.Segment1:FindFirstChild("ClickDetector") then
                                        if game:GetService("Workspace").Map["Haunted Castle"]["Lab Puzzle"].ColorFloor.Model.Part1:FindFirstChild("ClickDetector") then
                                            Quest4 = true
                                            repeat task.wait() topos(CFrame.new(-9553.5986328125, 65.62338256835938, 6041.58837890625)) until (CFrame.new(-9553.5986328125, 65.62338256835938, 6041.58837890625).Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 3 or not _G.AutoNevaSoulGuitar
                                            task.wait(1)
                    ...
                    if game:GetService("Workspace").Enemies:FindFirstChild("Forest Pirate") then
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v.Name == "Forest Pirate" then
                                ...
                                until _G.AutoObservationHakiV2 == false or SafeHumanoidHealth(v) <= 0
    ...
    pcall(function()
        game:GetService("RunService").Heartbeat:Connect(function()
            ...
            if _G.AutoObservationHakiV2 and StatrMagnet then
                for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
                    if v.Name == "Forest Pirate" and v:FindFirstChild("Humanoid") and SafeHumanoidHealth(v) > 0 then
                        if v.Name == "Forest Pirate" then
                            v.HumanoidRootPart.CanCollide = false
                            v.HumanoidRootPart.Size = Vector3.new(50, 50, 50)
                            v.HumanoidRootPart.CFrame = PosHee
                    ...
                    if v:FindFirstChild("HazeESP") then
                        repeat task.wait()
                            if (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 2000 then
                                ...
                                MonFarm = v.Name
                                Click()
                                if SafeHumanoidHealth(v) <= 0 and v.Humanoid:FindFirstChild("Animator") then
                                    pcall(function() SafeDestroyAnimator(v) end)
                                end
                            end
                        until Auto_Cursed_Dual_Katana == false or Auto_Quest_Yama_2 == false or not v.Parent or SafeHumanoidHealth(v) <= 0 or not v:FindFirstChild("HazeESP")
                    else
                            ...
                            for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v.Name == "Cursed Skeleton" or v.Name == "Cursed Skeleton" or v.Name == "Hell's Messenger]" then
                                    if SafeHumanoidHealth(v) > 0 then
                                        ...
                                        until SafeHumanoidHealth(v) <= 0 or not v.Parent or Auto_Quest_Yama_3 == false
                                    end
                                end
                            end
                        else
    ...
    while task.wait() do
        if Auto_Quest_Tushita_2 then
            pcall(function()
                if (CFrame.new(-5539.3115234375, 313.800537109375, -2972.372314453125).Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 500 then
                    for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                        if Auto_Quest_Tushita_2 and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and SafeHumanoidHealth(v) > 0 then
                            if (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude < 2000 then
                                repeat task.wait()
                                    EquipWeapon(Sword)
                                    topos(v.HumanoidRootPart.CFrame * Pos)
                                ...
                                until SafeHumanoidHealth(v) <= 0 or not v.Parent or Auto_Quest_Tushita_2 == false
                            end
                        end
                    end
                else
                            ...
                            for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                                if v.Name == "Cursed Skeleton" or v.Name == "Cursed Skeleton" or v.Name == "Heaven's Guardian" then
                                    if SafeHumanoidHealth(v) > 0 then
                                        ...
                                        until SafeHumanoidHealth(v) <= 0 or not v.Parent or Auto_Quest_Tushita_3 == false
                                    end
                                end
                            end
                        else
                            ...
                            if v.Name == "Longma" then
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                    repeat task.wait()
                                        ...
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                        sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                    until not  _G.Autotushita or not v.Parent or SafeHumanoidHealth(v) <= 0
                                end
                            end
                        end
                    else
                            ...
                            if v.Name == "Beautiful Pirate" then
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                    repeat task.wait()
                                        ...
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                        sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                    until not  _G.AutoCarvender or not v.Parent or SafeHumanoidHealth(v) <= 0
                                end
                            end
                        end
                    else
                            ...
                            if v.Name == "Captain Elephant" then
                                if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                    repeat task.wait()
                                        ...
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                        sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                    until not  _G.AutoTwinHook or not v.Parent or SafeHumanoidHealth(v) <= 0
                                end
                            end
                        end
                    else
                                    ...
                                    if v.Name == "Training Dummy" or v.Name == "Training Dummy" or v.Name == "Training Dummy" then
                                        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                            repeat task.wait()
                                                ...
                                                game:GetService("VirtualUser"):CaptureController()
                                                game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                            until _G.Namfon == false or SafeHumanoidHealth(v) <= 0 or not v.Parent
                                            Fastattack = false
                                        end
                                    end
                                end
                            ...
                            if v.Name == "Reborn Skeleton" or v.Name == "Living Zombie" or v.Name == "Demonic Soul" or v.Name == "Posessed Mummy" then
                               if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                                   repeat task.wait()
                                        ...
                                        game:GetService("VirtualUser"):CaptureController()
                                        game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                    until not _G.Auto_Bone2 or not v.Parent or SafeHumanoidHealth(v) <= 0
                                end
                            end
                        end
                    else
                        ...
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if string.find(v.Name , "Soul Reaper") then
                                repeat task.wait()
                                    ...
                                    v.HumanoidRootPart.Transparency = 1
                                    sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",math.huge)
                                until SafeHumanoidHealth(v) <= 0 or _G.AutoFarmBossHallow == false
                            end
                        end
                    elseif game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Hallow Essence") or game:GetService("Players").LocalPlayer.Character:FindFirstChild("Hallow Essence") then
                        ...
                        if game:GetService("ReplicatedStorage"):FindFirstChild("Soul Reaper") then
                            ...
                            if CheckItem("Hallow Scythe") then
                                Library.Flags["ToggleHallow"] = false
                                _G.Auto_Bone2 = false
                                StopTween(_G.Auto_Bone)
                                _G.Auto_Random_Bone2 = false
                                end
            ...
            while task.wait() do
                if _G.AutoDarkDagger then
                    if game:GetService("Workspace").Enemies:FindFirstChild("rip_indra True Form") or game:GetService("Workspace").Enemies:FindFirstChild("rip_indra") then
                        for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v.Name == ("rip_indra True Form" or v.Name == "rip_indra") and SafeHumanoidHealth(v) > 0 and v:IsA("Model") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
                                repeat task.wait()
                                    pcall(function()
                                        AutoHaki()
                                        EquipWeapon(_G.SelectWeapon)
                                ...
                                until _G.AutoDarkDagger == false or SafeHumanoidHealth(v) <= 0
                            end
                        end
                    else
                    if BypassTP then
			 ...
			 then
				if getBlueGear() then
					...
					if BlueGear and not BlueGear.CanCollide and BlueGear.Transparency ~= 1 then
						...
						if game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CheckTempleDoor") then
							DaGatCan = true
							Library.Flags["MoveCamTooMoonToggle"] = false
							Library.Flags["AutoPullLever"] = false
						end
                        ...
                        if v.Name == "Cocoa Warrior" or v.Name == "Chocolate Bar Battler" or v.Name == "Sweet Thief" or v.Name == "Candy Rebel" then
                           if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                               repeat task.wait()
                                    ...
                                    game:GetService("VirtualUser"):CaptureController()
                                    game:GetService("VirtualUser"):Button1Down(Vector2.new(1280,672))
                                until not StardFarm or not v.Parent or SafeHumanoidHealth(v) <= 0
                            end
                        end
                    end
                else
                        ...
                        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and SafeHumanoidHealth(v) > 0 then
                            pcall(function()
                                repeat task.wait(.1)
                                    ...
                                    v.HumanoidRootPart.CanCollide = false
                                    sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
                                until not _G.AutoQuestRace or not v.Parent or SafeHumanoidHealth(v) <= 0
                            end)
                        end
                    end
                elseif game:GetService("Players").LocalPlayer.Data.Race.Value == "Skypiea" then
...
plyselecthunthelpold = SafeHumanoidHealth(v)
repeat task.wait()
NameTarget = v.Name
AutoHaki()
EquipWeapon(_G.SelectWeapon)
...
v.HumanoidRootPart.CanCollide = false
TargetSelectHunt = SafeGetHumanoid(v)
until KillPlayer == false or SafeHumanoidHealth(v) == 0 or not v:FindFirstChild("HumanoidRootPart") or not v:FindFirstChild("Humanoid") or not v.Parent or NextplySelect == true
NextplySelect = false
...
notify("Versus Airlines", "Loading complete!", "info")
